...
def main():
    ...
    if args.dataset is not None and (args.dataset.endswith('.json') or args.dataset.endswith('.jsonl')):
        ...
        # If no dataset provided, fall back to defaults
    ...
    # Define compute_metrics function to compute both ROC AUC and accuracy
    if args.task == 'nli':
        def compute_metrics(eval_preds):
            """
            Compute both ROC AUC and accuracy for NLI evaluation.
            """
            # Predictions and ground truth labels
            preds = np.argmax(eval_preds.predictions, axis=1)  # Predicted labels
            y_true = eval_dataset['label']  # True labels
            
            # Accuracy
            accuracy = (preds == y_true).mean()
            
            # ROC AUC
            probs = np.exp(eval_preds.predictions)  # Convert logits to probabilities
        ...
        # For QA tasks, use default SQuAD metric
